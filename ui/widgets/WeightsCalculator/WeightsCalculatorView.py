...
class WeightCalculatorView(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.main_layout.addWidget(QLabel("Methode de calcul de poids:"))
    ...
    def _update_preview_plot(self, weight_fn: WeightFunction, n_runs: int, vects: List[np.ndarray], vects_rand: List[np.ndarray], const: int = 1):
        self.figure.clear()
        ...
        ax.plot(avg_weights, label=f"Cluster")
        ax.plot(avg_rand, label=f"Random")

        ax.set_title("Evolution du Poid selon la génération")
